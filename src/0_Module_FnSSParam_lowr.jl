...
"""
function FnSSParam_lowr(lsspar, Twork, Tret, ngpm, ngpz, ngpe, ngpal, kappa, aldist, algrid, edist, egrid, zdist, zgrid, Display, BendPointsPostTax, ScaleBendPoints, UseFinalZPension, MatchAggPensionBen, avearnspre, avearnspost, mgrid, pencap, targetSSBenToLabinc, targetSSAvReplacement)
                ...
                while ial<=ngpal
                    ...
                    while it<=Tret
                        if UseFinalZPension==1
                            ...
                            lmearns = exp(g + algrid[ial] + egrid[Twork, ie] + zgrid[Twork, iz])
                            lmearns = mgrid[Twork, im]
                            if lmearns <= ssbend1
                                ppregrid[it, ip] = 0.5*lmearns
                            elseif lmearns <= ssbend2
                                ppregrid[it, ip] = 0.5*ssbend1 + 0.5*(lmearns - ssbend1)
                            else
                                ppregrid[it, ip] = 0.5*ssbend1 + 0.5*(ssbend2-ssbend1) + 0.5*(lmearns - ssbend2)
                            end
                        else
                            lmearns = mgrid[Twork, im]
                            if lmearns <= ssbend1
                        ...
                        pgrid[it, ip] = ppregrid[it, ip] - FnTax
                        it=it+1
                    end
                    ip = ip+1
                    ial=ial+1
    ...
    if MatchAggPensionBen==1
        ...
        if UseFinalZPension==1
            if lmearns <= ssbend1
                lmpen = 0.5*lmearns
            elseif lmearns <= ssbend2
                lmpen = 0.5*ssbend1 + 0.5*(lmearns - ssbend1)
            else
                lmpen = 0.5*ssbend1 + 0.5*(ssbend2-ssbend1) + 0.5*(lmearns - ssbend2)
            end
        else
            if lmearns <= ssbend1
                lmpen = 0.9*lmearns
